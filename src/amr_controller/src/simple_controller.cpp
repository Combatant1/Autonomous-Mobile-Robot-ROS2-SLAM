...
void SimpleController::velCallback(const geometry_msgs::msg::TwistStamped & msg)
{
    ...
    wheel_speed_msg.data.push_back(wheel_speed.coeff(0)); // Right wheel speed -> front_right_joint
    wheel_speed_msg.data.push_back(wheel_speed.coeff(1)); // Left wheel speed  -> front_left_joint
    wheel_speed_msg.data.push_back(wheel_speed.coeff(0)); // Right wheel speed -> rear_right_joint
    wheel_speed_msg.data.push_back(wheel_speed.coeff(1)); // Left wheel speed  -> rear_left_joint
...
void SimpleController::jointCallback(const sensor_msgs::msg::JointState & msg)
{
 ...
 transform_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
    transform_stamped_.header.frame_id = "odom";
    transform_stamped_.child_frame_id = "base_footprint";   double d_theta = (wheel_radius_ * dp_front_right - wheel_radius_ * dp_front_left) / wheel_separation_;
